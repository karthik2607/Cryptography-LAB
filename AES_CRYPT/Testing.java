...
public class Testing {
    ...
    private void addRoundKey(byte[] state, int round) {
        for (int i = 0; i < 4; i++) {
            int word = expandedKey[round * Nb + i];
            for (int j = 0; j < 4; j++) {
                state[i * 4 + j] ^= (byte) ((word >> (24 - j * 8)) & 0xFF);
            }
        }
    }
    ...
    private void mixColumns(byte[] state) {
        for (int i = 0; i < 4; i++) {
            int a = state[4 * i] & 0xFF;
            int b = state[4 * i + 1] & 0xFF;
            int c = state[4 * i + 2] & 0xFF;
            int d = state[4 * i + 3] & 0xFF;

            int e = a ^ b ^ c ^ d;

            int tempA = a ^ e ^ mul2(a ^ b);
            int tempB = b ^ e ^ mul2(b ^ c);
            int tempC = c ^ e ^ mul2(c ^ d);
            int tempD = d ^ e ^ mul2(d ^ a);

            state[4 * i] = (byte) tempA;
            state[4 * i + 1] = (byte) tempB;
            state[4 * i + 2] = (byte) tempC;
            state[4 * i + 3] = (byte) tempD;
        }
    }
    ...
    private void invMixColumns(byte[] state) {
        for (int i = 0; i < 4; i++) {
            ...
            int tempA = mul(0x0e, a) ^ mul(0x0b, b) ^ mul(0x0d, c) ^ mul(0x09, d);
            int tempB = mul(0x09, a) ^ mul(0x0e, b) ^ mul(0x0b, c) ^ mul(0x0d, d);
            int tempC = mul(0x0d, a) ^ mul(0x09, b) ^ mul(0x0e, c) ^ mul(0x0b, d);
            int tempD = mul(0x0b, a) ^ mul(0x0d, b) ^ mul(0x09, c) ^ mul(0x0e, d);
